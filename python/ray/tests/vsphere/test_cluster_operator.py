...
from python.ray.autoscaler.tags import NODE_KIND_HEAD, NODE_KIND_WORKER, STATUS_SETTING_UP, STATUS_UNINITIALIZED, STATUS_UP_TO_DATE, TAG_RAY_LAUNCH_CONFIG, TAG_RAY_NODE_KIND, TAG_RAY_NODE_NAME, TAG_RAY_NODE_STATUS, TAG_RAY_USER_NODE_TYPE
from ray.autoscaler._private.vsphere.cluster_operator_client import DEFAULT_WORKER_NODE_TYPE, ClusterOperatorClient, VMNodeStatus
...
def test_list_vms():
    ...
    cluster_response["status"]["head_node_status"]["vm_status"] = VMNodeStatus.INITIALIZED.value
    # Mock cluster response
    ...
    cluster_response["status"]["head_node_status"]["vm_status"] = VMNodeStatus.FAIL.value
    # Mock cluster response
    operator._get_cluster_response = MagicMock(return_value = cluster_response)
    # make sure the function provides head node only
    nodes, tag_cache = operator.list_vms(head_node_tags)
    ...
    tag_filters = {}
    cluster_response = _CLUSTER_RESPONSE.copy()
    cluster_response["status"]["head_node_status"]["vm_status"] = VMNodeStatus.RUNNING.value
    # Mock cluster response
    operator._get_cluster_response = MagicMock(return_value = cluster_response)
    ...
    for node in nodes:
        ...
        assert tag_cache[node][TAG_RAY_NODE_KIND] == NODE_KIND_WORKER
        assert tag_cache[node][TAG_RAY_USER_NODE_TYPE] == DEFAULT_WORKER_NODE_TYPE
    # make sure the function returns nodes which are not yet initialised but are in desired
    # workers list
    cluster_response = _CLUSTER_RESPONSE.copy()
    del cluster_response["status"]["current_workers"]["test-cluster-worker-444d0450-0d67-4180-9bd9-d43ee02b5186"]
    # Mock cluster response
    operator._get_cluster_response = MagicMock(return_value = cluster_response)
    nodes, tag_cache = operator.list_vms(tag_filters)
...
def test_is_vm_power_on():
    ...
    operator = mock_cluster_operator()
    operator.lock = RLock()
    operator._get_node = MagicMock(return_value = {"vm_status": VMNodeStatus.RUNNING.value})
    assert operator.is_vm_power_on("test_vm") is True
    operator._get_node = MagicMock(return_value = {"vm_status": VMNodeStatus.INITIALIZED.value})
...
def test_get_vm_external_ip():
    ...
    operator = mock_cluster_operator()
    operator.lock = RLock()
    operator._get_node = MagicMock(return_value = {"vm_status": VMNodeStatus.RUNNING.value, "ip": "10.10.10.10"})
    ip = operator.get_vm_external_ip("test_vm")
    assert ip == "10.10.10.10"
    operator._get_node = MagicMock(return_value = {"vm_status": VMNodeStatus.RUNNING.value, "ip": ""})
    ip = operator.get_vm_external_ip("test_vm")
    assert ip  is None
    operator._get_node = MagicMock(return_value = {"vm_status": VMNodeStatus.FAIL.value, "ip": ""})
    ip = operator.get_vm_external_ip("test_vm")
    assert ip  is None
...
def test_safe_to_scale():
    ...
    operator.max_worker_nodes = 5
    cluster_response = _CLUSTER_RESPONSE.copy()
    # Should not scale until current state matched desired state

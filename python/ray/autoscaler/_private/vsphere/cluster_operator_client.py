...
class ClusterOperatorClient(IKubernetesHttpApiClient):
    def __init__(self, provider_config: Dict[str, Any]):
        ...
        ), f"To use vSphereNodeProvider, must set SERVICE_ACCOUNT_TOKEN env variable."
    ...
    def list_vms(self, tag_filters: Dict[str, str]) -> list:
        ...
        with self.lock:
            ...
            vmray_cluster_status = self._get(f"vmraycluster/status")
    ...
    def delete_node(self, nodeId: str) -> None:
        ...
        the VmRayCluster CR"""
        with self.lock:
            vmray_cluster_spec = self._get(f"vmraycluster/spec")
            # get desired workers 
    ...
    def create_nodes(
        ...
        with self.lock:
            if to_be_launched_node_count > 0:
                ...
                new_vm_names = [
                    "{}_{}_{}".format(self.cluster_name, tags[TAG_RAY_NODE_NAME], str(uuid.uuid4()))
                ...
                new_desired_workers.extend(current_workers,)
